...
/**
 * One person's library entry: their photographs, what they agreed those
 * photographs may be used for, and their comp card.
 *
...
export default function TalentLibrary({ t }: { t: Talent }) {
  ...
  return (
    <section className="lib" aria-label={`${t.name.en}: library`}>
      ...
      <p className="hint">
        Each is a separate agreement with its own end date. A photo reaches a client
        or the website only while the matching one is live, and withdrawing one never
        changes how often they are cast.
      </p>
      <div className="lib-consents">
        {PURPOSES.map((p) => {
          const c = latestConsent(t.consents, p);
          const live = consentLive(t.consents, p, now);
          const needsRoster = p !== 'roster' && !consentLive(t.consents, 'roster', now);
            ...
            <div key={p} className="lib-consent" data-state={state}>
              ...
              <div className="row">
                {live ? (
                  ...
                  <button disabled={busy || needsRoster}
      ...
      <div className="row" style={{ marginTop: 10 }}>
        <input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp" multiple
               disabled={busy || !canSee} style={{ maxWidth: 320 }}
               onChange={(e) => upload(e.target.files)} />
      </div>
      {!canSee && (
        <p className="hint">Uploading opens once “Our records” consent is recorded.</p>
      )}
      ...
      <p className="hint">Everything optional. None of this reaches a client or the website.</p>
      <div className="lib-card">
        {fields.map((f) => (
          <div key={f.key} className="lib-field" data-kind={f.kind}>
            <label htmlFor={`${t.id}-${f.key}`}>
              {f.label.en}{f.kind === 'number' && f.unit ? ` (${f.unit.en})` : ''}
            </label>
            {f.kind === 'choice' ? (
              <select id={`${t.id}-${f.key}`} className="sel" value={valueOf(f)} disabled={busy}
                      onChange={(e) => setDraft((d) => ({ ...d, [f.key]: e.target.value }))}>
                <option value="">—</option>
                {f.options.map((o) => <option key={o.value} value={o.value}>{o.label.en}</option>)}
              </select>
            ) : f.kind === 'flag' ? (
              <input id={`${t.id}-${f.key}`} type="checkbox" checked={valueOf(f) === 'true'} disabled={busy}
                     onChange={(e) => setDraft((d) => ({ ...d, [f.key]: String(e.target.checked) }))} />
            ) : (
              <input id={`${t.id}-${f.key}`} value={valueOf(f)} disabled={busy}
                     inputMode={f.kind === 'number' ? 'decimal' : undefined}
                     dir={f.kind === 'number' ? 'ltr' : 'auto'}
                     placeholder={f.kind === 'list' ? (f.suggest ?? []).slice(0, 3).join(', ') : ''}
                     onChange={(e) => setDraft((d) => ({ ...d, [f.key]: e.target.value }))} />
            )}
            {f.hint && <span className="hint">{f.hint.en}</span>}
          </div>
        ))}
      </div>
      <div className="row">
